...
static SoftwareSerial *s_SoftSerial;
static HardwareSerial *s_HardSerial;
static int s_SoftFlag = 0;
static int s_HardFlag = 0;
...
static int SendInst(uint8_t *inst, int instLen)
{
    ...
    for (i = 0; i < instLen; i++){
        if (s_SoftFlag)
          s_SoftSerial->write(inst[i]);
        if (s_HardFlag)
          s_HardSerial->write(inst[i]);
    }
  return 0;
}
...
PANDA_UI::PANDA_UI(SoftwareSerial *softSerial, HardwareSerial *hardSerial)
{
    if (NULL != softSerial){
      s_SoftSerial = softSerial;
      s_SoftFlag = 1;
    }

    if (NULL != hardSerial){
      s_HardSerial = hardSerial;
      s_HardFlag = 1;  
    }
}
